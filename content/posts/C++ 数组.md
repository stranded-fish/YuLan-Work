...
> **扩展：** 当指定范围内包含多个相等的元素时，`sort()` 排序函数无法保证不改变它们的相对位置。如果既要完成排序又要保证相等元素的相对位置，可以使用 `stable_sort()` 函数。`stable_sort()` 函数与 `sort()` 函数在使用接口上完全相等。
>
> * 典型场景：一个保存某事务（银行账号）的容器，在处理这些事务之前，需要按照银行账号进行排序，此时可能会出现多个相同的账号（即同一账号多个事务），为了能够保持事务的有序，需要保证排序过程中相同账号的相对顺序不能发生改变。
> * 典型例题：[937. 重新排列日志文件](https://leetcode-cn.com/problems/reorder-data-in-log-files/)
>
> 更多排序函数可参见：[C++ sort()排序函数用法详解](http://c.biancheng.net/view/7457.html)
...
* [C++ sort()排序函数用法详解](http://c.biancheng.net/view/7457.html)
